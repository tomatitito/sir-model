...
(defn inc-actual-changes [ref]
  ...
  trigger the creation of a new thread that runs the cohort function."
  (update ref :actual inc) )

(defn cohort [current-infections recovery-rate I-map I-counter-map R-map current-timestep max-timesteps]
  ...
  (if (< current-timestep max-timesteps)
    (if (pos? current-infections)                                  ;are there still infectied people in this group
      ...
      (let [cured (min current-infections (sample* (binomial current-infections recovery-rate)))
        ...
        (cohort still-inf recovery-rate I-map I-counter-map R-map (inc current-timestep) max-timesteps)))))


(defn compute-new-infections [I-map S-map R-0 current-timestep]
...
;(defn watch-I-counter [key watched old-state new-state]
;  "Checks if all threads have made their changes to the number of current infections. If so,
;  new infections can be computed by compute-new-infections."
;  (if (= new-state (dec current-timestep) )
;    (compute-new-infections I-map S-map R-0 current-timestep)))
